...
production_biomass <- 
  unique(x = subset(x = read.csv(file = "temp/production/production_biomass_EBS.csv"),
                    subset = YEAR == 2023 & !(AREA_ID %in% c(99901, 101, 201, 301))))
...
production_sizecomp <- 
  unique(x = subset(x = read.csv(file = "temp/production/production_sizecomp_EBS.csv"),
                    subset = YEAR == 2022 & !(AREA_ID %in% c(99901, 101, 201, 301)),
...
production_agecomp <- 
  unique(x = subset(x = read.csv(file = "temp/production/production_agecomp_EBS.csv"),
                    subset = YEAR == 2022 & AREA_ID != 99901,
...
historical_sizecomp <-
  RODBC::sqlQuery(channel = sql_channel,
                  query = paste0(
                    ...
                    " FROM HAEHNR.SIZECOMP_EBS_PLUSNW_STRATUM WHERE YEAR = 2022"))
...
unique(historical_biomass$SPECIES_CODE)[!unique(historical_biomass$SPECIES_CODE) %in% unique(production_biomass$SPECIES_CODE)]
...
test <- merge(x = subset(production_alk, 
                         YEAR == 2022 & SPECIES_CODE == 21740 & AGE %in% c(4) & SEX == 1),
              y = subset(historical_sizecomp, 
                         subset = AREA_ID == 99900 & YEAR == 2022 &
                           SPECIES_CODE %in% unique(x = historical_agecomp$SPECIES_CODE)),
              by = c("YEAR", "SPECIES_CODE", "SEX", "LENGTH_MM"))
subset(test_agecomp, SPECIES_CODE == 21740 & AGE == 4 & SEX == 1 & AREA_ID == 99900)
