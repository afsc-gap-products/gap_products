...
production_data <- readRDS(file = "temp/production_data_GOA.RDS")

## CPUE
production_cpue <- read.csv(file = "temp/production_cpue_GOA.csv")

## strata information
production_strata <- read.csv(file = paste0("temp/production_strata_GOA.csv"))

## subarea information
production_subarea <- read.csv(file = paste0("temp/production_subarea_GOA.csv"))

## stratum biomass
production_biomass <- 
  unique(x = subset(x = read.csv(file = "temp/production_biomass_GOA.csv"),
...
production_sizecomp0 <- 
  unique(x = subset(x = read.csv(file = "temp/production_sizecomp_GOA.csv"),
                    subset = AREA_ID %in% production_strata$STRATUM))
## Change AREA_ID field name to STRATUM
names(x = production_sizecomp0)[names(x = production_sizecomp0) == "AREA_ID"] <- 
  "STRATUM" 
## Age-length key
production_alk <- read.csv(file = paste0("temp/production_alk_GOA.csv"))
# age composition
production_agecomp <- 
  unique(x = subset(x = read.csv(file = "temp/production_agecomp_GOA.csv"),
