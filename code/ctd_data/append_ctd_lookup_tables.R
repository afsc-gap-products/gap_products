...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
env <- readRDS(file = "code/ctd_data/GAPCTD_all_casts_2021_2024.rds")
names(x = env) <- toupper(x = names(x = env))

instruments <- readRDS(file = "code/ctd_data/GAPCTD_instrument_2021_2024.rds")
...
env <- merge(
  ...
  y = instruments, 
  by = "HAULJOIN", 
  all.x = TRUE # Keeps all rows from ctd_data even if a hauljoin is missing in the instrument table
)
