...
REM ***NEED TO RUN SIZECOMP_PLUSNW_YR_STRATUM.SQL BEFORE RUNNING THIS SCRIPT***
...
select distinct species_code, year, length from sizecomp_ebs_plusnw_stratum ;
...
(decode(sum(ratm*males),0,1,sum(ratm*males))))/(decode(sum(ratm*males)-1,0,1,sum(ratm*males)-1)),8)) sdev
 from sizecomp_ebs_plusnw_stratum sz, ratioma r
...
(decode(sum(ratf*females),0,1,sum(ratf*females))))/(decode(sum(ratf*females)-1,0,1,sum(ratf*females)-1)),8)) sdev
 from sizecomp_ebs_plusnw_stratum sz, ratiofe r
where sz.species_code=r.species_code and sz.year=r.year and sz.length=r.length 
group by sz.species_code, sz.year, stratum, age, sex;
...
(decode(sum(ratu*unsexed),0,1,sum(ratu*unsexed)))))/((decode(sum(ratu*unsexed)-1,0,1,sum(ratu*unsexed))-1)),8)) sdev
 from sizecomp_ebs_plusnw_stratum sz, ratioun r
where sz.species_code=r.species_code and sz.year=r.year and sz.length=r.length 
group by sz.species_code, sz.year, stratum,age,sex;
...
select s.species_code, s.year, stratum, r.length, r.age, ratm*males pops 
 from ratioma r, sizecomp_ebs_plusnw_stratum s where s.species_code=r.species_code and s.year=r.year and r.length=s.length ;

drop table fetest;
drop view fetest;
create view fetest as
select s.species_code, s.year, stratum, r.length,r.age, ratf*females pops from
ratiofe r, sizecomp_ebs_plusnw_stratum s where s.species_code=r.species_code and s.year=r.year and r.length=s.length ;

drop table untest;
drop view untest;
create table untest as
select s.species_code, s.year, stratum, r.length,r.age, ratu*unsexed pops from
ratioun r, sizecomp_ebs_plusnw_stratum s where s.species_code=r.species_code and s.year=r.year and r.length=s.length ;
...
drop table agecomp_ebs_plusnw_stratum;
create table agecomp_ebs_plusnw_stratum as
select * from agemale;

insert into agecomp_ebs_plusnw_stratum
...
update agecomp_ebs_plusnw_stratum set sdev=null where meanlen=0;
update agecomp_ebs_plusnw_stratum set meanlen=null where meanlen=0;
...
from sizecomp_ebs_plusnw_stratum where length=-9 order by 
species_code, year, stratum;
update missing_age_pop set meanlen=null;
update missing_age_pop set sdev=null;
insert into agecomp_ebs_plusnw_stratum select * from missing_age_pop;
REM ********************************************************************************************************************************


grant select on agecomp_ebs_plusnw_stratum to public;
