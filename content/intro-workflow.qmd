...
```{r}
dat <- data.frame(matrix(data = c(
  "TABLE NAMES" ,"TABLE TYPE","REFRESH FREQUENCY",
  "AGECOMP\nBIOMASS\nCPUE\nSIZECOMP",
  "TABLE",
  "Quarterly: manually updated according to the following schedule\nLate Summer: after survey data are finalized for each region surveyed\nEarly Fall: after Bering Sea pollock otoliths are read\nWinter: If there are changes to the taxonomic tables\nLate Spring: after Spring ages are updated","CRUISE","HAUL","LENGTH","SPECIMEN",
  "MATERIALIZED VIEW",
  "As needed: Refreshes any time the underlying tables for these queries change",
  "SURVEY_DESIGN\nTAXONOMIC_CLASSIFICATION\nTAXONOMIC_GROUPS\nTAXONOMIC_CHANGES\nTAXON_GROUPS",
  "TABLE",
  "Annually (beginning of year): manually updated.",
  "AREA\nSTRATUM_GROUPS\nMETADATA_COLUMN\nMETADATA_TABLE\nSPECIES_YEAR",
  "TABLE","As needed: manually updated.",
  "SAMPLE_COUNT",
  "VIEW",
  "When queried."), ncol = 3, byrow = TRUE)
)
names(dat) <- dat[1,]
dat <- dat[2:nrow(dat),] 
