...
REM THIS SCRIPT PRODUCES POPULATION NUMBERS AT LENGTH BY SPECIES_CODE, YEAR, STRATUM (10,20,31,32,41,42,43,50,61,62,82,90).
...
select t.species_code, t.year, t.stratum, -9 length, 0 males, 0 females, 0 unsexed, sum(b.population) total 
 from biomass_ebs_plusnw b, temp_catlen t 
...
drop table sizecomp_ebs_plusnw_stratum;
create table sizecomp_ebs_plusnw_stratum as 
...
delete from sizecomp_ebs_plusnw_stratum where year<=1986;


drop table temp_sizecomp;

grant select on sizecomp_ebs_plusnw_stratum to public;

select * from sizecomp_ebs_plusnw_stratum order by species_code, year, stratum, length;