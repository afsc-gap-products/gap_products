...
REM THIS SCRIPT PRODUCES POPULATION NUMBERS AT LENGTH BY SPECIES_CODE, YEAR, STRATA (70, 71, 81, 999999).
REM INCLUDES THE STANDARD + NW STRATA.

REM modified 9/14/22 updated racebase.stratum year = 2022 (area), HAEHNR
...
REM ***************THIS SCRIPT REQUIRES YOU RUM BIOMASS_NBS_STRATUM.SQL PRIOR TO RUNNING THIS SCRIPT, WHICH UPDATES ORACLE TABLE BIOMASS_NBS_SAFE***************
...
(435,471,472,
10110,10112,10115,10120,10130,10140,10155,10180,10200,10210,10211,10212,10220,10260,10261,10262,10270,10285,
...
from racebase.length l, haul_nbs h 
...
year,stratum,hauljoin,haul from haul_nbs;
drop table temp2;
...
select species_code,s.year,s.stratum, (avgcpue * area * 100) population
 from stratavg s, racebase.stratum a 
where 
  s.stratum = a.stratum and 
  a.region = 'BS' and 
  a.year= 2022 and 
  a.stratum in (70,71,81);
...
REM **********************************************************************************************************************************
REM THIS SECTION FINDS THE CASES IN WHICH CATCH NUMBERS EXISTED IN A STRATUM BUT NO LENGTHS WERE MEASURED,
...
select c.species_code, h.year, h.stratum, sum(number_fish) num_fish from haul_nbs h, racebase.catch c
where 
h.hauljoin=c.hauljoin and number_fish>0  and 
species_code in (435,471,472,
10110,10112,10115,10120,10130,10140,10155,10180,10200,10210,10211,10212,10220,10260,10261,10262,10270,10285,
21420,21371,21370,21368,21347,
21720,21725,21735,21740)
...
create table temp_length as select l.species_code, h.year, h.stratum, sum(frequency) num_lengths from haul_nbs h, racebase.length l 
where h.hauljoin=l.hauljoin and frequency>0 
and species_code in (435,471,472,
10110,10112,10115,10120,10130,10140,10155,10180,10200,10210,10211,10212,10220,10260,10261,10262,10270,10285,
21420,21371,21370,21368,21347,
21720,21725,21735,21740)
...
select t.species_code, t.year, t.stratum, -9 length, 0 males, 0 females, 0 unsexed, sum(b.population) total 
 from biomass_nbs_safe b, temp_catlen t 
...
drop table sizecomp_nbs_stratum;
create table sizecomp_nbs_stratum as 
...
REM delete from sizecomp_nbs_stratum where subarea=9;

select a.species_code, b.species_name, b.common_name, year, STRATUM, length,
round(males) males, round(females) females, round(unsexed) unsexed, round(total) total  
from sizecomp_nbs_stratum a, racebase.species b
where a.species_code=b.species_code 
and a.species_code in (435,471,472,
10110,10112,10115,10120,10130,10140,10155,10180,10200,10210,10211,10212,10220,10260,10261,10262,10270,10285,
21420,21371,21370,21368,21347,
21720,21725,21735,21740)
...
grant select on sizecomp_nbs_stratum to public;
commit;
