...
REM ***NEED TO RUN SIZECOMP_SIZECOMP_BY_STRATUM.SQL BEFORE RUNNING THIS SCRIPT***
...
from racebase.specimen s, haul_nbs h 
...
select distinct species_code, year, length from sizecomp_nbs_stratum ;

REM THIS MAKES A LIST OF THE SPECIES AND YEARS IN WHICH AGES WERE COLLECTED
drop table speclist;
drop view speclist;
create table speclist as select distinct s.species_code, year from racebase.specimen s, haul_nbs h where 
...
(decode(sum(ratm*males),0,1,sum(ratm*males))))/(decode(sum(ratm*males)-1,0,1,sum(ratm*males)-1)),8)) sdev
 from sizecomp_nbs_stratum sz, ratioma r
...
(decode(sum(ratf*females),0,1,sum(ratf*females))))/(decode(sum(ratf*females)-1,0,1,sum(ratf*females)-1)),8)) sdev
 from sizecomp_nbs_stratum sz, ratiofe r
where sz.species_code=r.species_code and sz.year=r.year and sz.length=r.length 
group by sz.species_code, sz.year, stratum, age, sex;
...
(decode(sum(ratu*unsexed),0,1,sum(ratu*unsexed)))))/((decode(sum(ratu*unsexed)-1,0,1,sum(ratu*unsexed))-1)),8)) sdev
 from sizecomp_nbs_stratum sz, ratioun r
where sz.species_code=r.species_code and sz.year=r.year and sz.length=r.length 
group by sz.species_code, sz.year, stratum,age,sex;
...
select s.species_code, s.year, stratum, r.length, r.age,ratm*males pops from 
ratioma r, sizecomp_nbs_stratum s where s.species_code=r.species_code and s.year=r.year and r.length=s.length ;

drop table fetest;
drop view fetest;
create view fetest as
select s.species_code, s.year, stratum, r.length,r.age, ratf*females pops from
ratiofe r, sizecomp_nbs_stratum s where s.species_code=r.species_code and s.year=r.year and r.length=s.length ;

drop table untest;
drop view untest;
create table untest as
select s.species_code, s.year, stratum, r.length,r.age, ratu*unsexed pops from
ratioun r, sizecomp_nbs_stratum s where s.species_code=r.species_code and s.year=r.year and r.length=s.length ;
...
drop table agecomp_nbs_stratum;
create table agecomp_nbs_stratum as
select * from agemale;

insert into agecomp_nbs_stratum
...
update agecomp_nbs_stratum set sdev=null where meanlen=0;
update agecomp_nbs_stratum set meanlen=null where meanlen=0;
...
from sizecomp_nbs_stratum where length=-9 order by 
species_code, year, stratum;
update missing_age_pop set meanlen=null;
update missing_age_pop set sdev=null;
insert into agecomp_nbs_stratum select * from missing_age_pop;
REM ********************************************************************************************************************************


grant select on agecomp_nbs_stratum to public;
