...
drop table samplesize_ebs_plusnw;
create table samplesize_ebs_plusnw as select * from hauls_w_length;
alter table samplesize_ebs_plusnw add total_hauls number;
alter table samplesize_ebs_plusnw add num_lengths number;
alter table samplesize_ebs_plusnw add hauls_w_otoliths number;
alter table samplesize_ebs_plusnw add hauls_w_ages number;
alter table samplesize_ebs_plusnw add num_otoliths number;
alter table samplesize_ebs_plusnw add num_ages number;


update samplesize_ebs_plusnw a set a.total_hauls=(select b.total_hauls from total_hauls b where a.year=b.year(+));
update samplesize_ebs_plusnw a set hauls_w_length=(select hauls_w_length from hauls_w_length b where a.species_code=b.species_code(+) 
and a.year=b.year(+));
update samplesize_ebs_plusnw a set num_lengths=(select num_lengths from num_lengths b where a.species_code=b.species_code(+) 
and a.year=b.year(+));
update samplesize_ebs_plusnw a set hauls_w_otoliths=(select hauls_w_otoliths from hauls_w_otoliths b where a.species_code=b.species_code(+) 
and a.year=b.year(+));
update samplesize_ebs_plusnw a set hauls_w_ages=(select hauls_w_ages from hauls_w_ages b where a.species_code=b.species_code(+) 
and a.year=b.year(+));
update samplesize_ebs_plusnw a set num_otoliths=(select num_otoliths from num_otoliths b where a.species_code=b.species_code(+) 
and a.year=b.year(+));
update samplesize_ebs_plusnw a set num_ages=(select num_ages from num_ages b where a.species_code=b.species_code(+) 
and a.year=b.year(+));


select species_code, year, total_hauls, hauls_w_length, num_lengths, hauls_w_otoliths, hauls_w_ages, num_otoliths, num_ages
 from samplesize_ebs_plusnw order by species_code, year;